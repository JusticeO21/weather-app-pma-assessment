...
def list_weather_records_by_location(location: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if not records:
            raise HTTPException(status_code=404, detail=f"No records found for location '{location}'")
        return {"success": True, "count": len(records), "data": records}
...
def save_weather_record(data: dict, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return {"success": True, "message": "Weather record created successfully", "data": record}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def update_weather_record(record_id: int, updates: dict, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if not updated:
            raise HTTPException(status_code=404, detail=f"Record with ID {record_id} not found")
        return {"success": True, "message": "Weather record updated successfully", "data": updated}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def delete_weather_record(record_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if not deleted:
            raise HTTPException(status_code=404, detail=f"Record with ID {record_id} not found")
        return {"success": True, "message": f"Record ID {record_id} deleted successfully"}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
