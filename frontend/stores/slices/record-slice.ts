...
import { fetchRecords, fetchRecord, deleteRecord, filterRecords } from '@/lib/api/records';
...
> = (set) => ({
    ...
    fetchRecords: async () => {
        try {
            set({ loading: { fetchAll: true, fetchOne: false, delete: false }, error: null });
    ...
    filterRecordsByLocation: async (location: string) => {
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Failed to filter records';
            showToast('error', errorMessage);
            set({
                error: errorMessage,
                loading: { fetchAll: false, fetchOne: false, delete: false },
    ...
    removeRecord: async (id) => {
        ...
        try {
            set({ loading: { delete: true, fetchAll: false, fetchOne: false }, error: null });
    ...
    getRecord: async (id: number) => {
        try {
            set({ loading: { fetchOne: true, fetchAll: false, delete: false }, error: null });
